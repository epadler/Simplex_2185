...
void MyMesh::GenerateCircle(float a_fRadius, int a_nSubdivisions, vector3 a_v3Color)
{
	...
	*/
	float rot = (2 * PI) / a_nSubdivisions;
	glm::mat4 rotationMat(1);
	rotationMat = glm::rotate(rotationMat, rot, glm::vec3(0, 0, 1));
	vector3 vec1 = vector3(0, 0, 0);
	vector3 vec2 = vector3(a_fRadius, 0, 0);
	vector3 vec3 = glm::vec3(rotationMat * glm::vec4(vec2 , 1.0));

	AddVertexPosition(vec1);
	AddVertexPosition(vec2);
	AddVertexPosition(vec3);
	for (int i = 0; i < a_nSubdivisions; i++)
	{
		vec2 = glm::vec3(rotationMat*glm::vec4(vec2, 1.0));
		vec3 = glm::vec3(rotationMat*glm::vec4(vec3, 1.0));
		AddVertexPosition(vec1);
		AddVertexPosition(vec2);
		AddVertexPosition(vec3);
	}
